         ...
         /**
           ...
           function removeTaxPayer(id){
        	   $.messager.confirm('操作提示', '确定要删除吗?', function(data){
	  				if (data){
	  					$.post("deleteTaxPayer.do",{"id":id},function(data){
			           		if(data.success){
			           			alert(data.msg)
			           			doSearch()	
			           		}else{
           ...
           //更新
    		function edit(id){	          
	   		 	openTopWindow({"url":"manage/jsp/editTaxpayer.jsp?id="+id,
	   		 	title:"修改与纳税人详情",	   		 	
	   		 	 width : 750,
                 height : 600,
				})
       ...
       $(function(){
		   ...
		   onClose:function(){
       	   ...
       	   columns:[[
                    ...
                    return "<a href='javascript:void(0)' onclick='detail("+rowData.id+")'>   </a>"
                    +"<a href='javascript:void(0)' onclick='removeTaxPayer("+rowData.id+")'>删除   </a>"
                    +"<a href='javascript:void(0)' onclick='edit("+rowData.id+")'>详细与修改   </a>"                               
                    }}   
       	   ]]      	          	   
       	   })
